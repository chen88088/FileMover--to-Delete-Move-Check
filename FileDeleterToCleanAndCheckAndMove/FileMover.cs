...
namespace FileDeleterToCleanAndCheckAndMove
{
    public class FileMover
    {
        ...
        /// 被加密檔案的路徑起頭
        /// </summary>
        public string[] EncryptionFilePathPrefixSet { get; set; }
        ...
        public FileMover(string backupSourcePathPrefix)
        {            
        ...
        public string CaptureFilePath(string EncryptionFilePath, string EncryptionPattern)
        {
            Match match = Regex.Match(EncryptionFilePath, EncryptionPattern);
            if (match.Success)
            {
